...
public class PathAgent : MonoBehaviour
{
    ...
    private void Start()
    {
        ...
        pathFinder = new AStarPathFinder(pathGrid, this.transform, target, obstacleLayers, ignorePathLayers, false);
    ...
    private void OnDrawGizmos()
    {
        ...
        for (int i = 0; i < nodes.GetLength(1); i++)
        {
            ...
            {
                Node node = nodes[i, j];
                if (node.isObstacle) Gizmos.color = Color.red;
                else if (node.dontIncludeToPath) Gizmos.color = Color.yellow;
                else if (node.ignoreable) Gizmos.color = Color.white;
                else if (finalPath.Contains(node)) Gizmos.color = Color.green;
